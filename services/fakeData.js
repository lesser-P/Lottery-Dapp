function generateLottery(id) {
  ...
  return {
    ...
    owner: generateRandomEthereumAddress(), //生成以太坊地址
    prize: getRandomFloat(10, 100).toFixed(2), //随机奖励
    ticketPrice: getRandomFloat(0.01, 0.1).toFixed(2), //随机价格
    image,
...
const generateLotteries = (n) => {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    const expiresIn = getRandomInt(7, 30)
    const expiresAt = new Date(Date.now() + expiresIn * 24 * 60 * 60 * 1000).getTime()
    const participants = getRandomInt(10, 100) //参与者数量
    const drawn = false
